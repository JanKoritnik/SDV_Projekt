...
void Mrf24j_init(uint8_t ch) // KAJ SE ZGODI V FUNKCIJI !!!!!!!!!!!!!!!!!!!!!!!
{
	...
	Mrf24j_write_short(MRF_SOFTRST, 0x7); // vse resetira, rabimo spet vse registre nastaviti
...
//  bytes_MHR = 3+2+2+2;
  ...
  uint16_t panid =0x1122;//= Mrf24j_get_pan();
  ...
  // Dest
  dest16=0x9988;
  Mrf24j_write_short(i++, dest16 & 0xff);  // dest16 low
  Mrf24j_write_short(i++, dest16 >> 8); // dest16 high
  // Dest
/*  dest16=0x9988;
  Mrf24j_write_short(i++, dest16 & 0xff);  // dest16 low
  Mrf24j_write_short(i++, dest16 >> 8); // dest16 high
